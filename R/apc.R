...
#'
#' The companion package \pkg{APCinteractionData} provides 90 pre-computed APCSSA null distributions (100,000 simulations each) covering two-way designs with 2 to 6 factor levels and 1 to 10 replications. Install it with
#' `devtools::install_github("tranbaokhue/APCinteractionData")` to enable fast p-value estimation for these designs.
#'
#' If the design is not covered by the pre-computed distributions or if a higher precision is desired (`numSim` > 100,000), users must first generate the null distribution using \code{\link{sim_nullAPCSSA}} before using this function to evaluate the interaction effect.
#'
...
#' These statistics use all possible crossed comparisons to detect interaction effects. The final test statistic is the maximum of the two standardized statistics. A *p*-value is estimated by comparing this statistic to a pre-simulated null distribution specific to the design dimensions.
#'
#' The companion package \pkg{APCinteractionData} provides 90 pre-computed APCSSM null distributions (100,000 simulations each) covering two-way designs with 2 to 6 factor levels and 1 to 10 replications. Install it with
#' `devtools::install_github("tranbaokhue/APCinteractionData")` to enable fast p-value estimation for these designs.
#'
#' If the design is not covered by the pre-computed distributions or if a higher precision is desired (`numSim` > 100,000), users must first generate the null distribution using \code{\link{sim_nullAPCSSM}} before using this function to evaluate the interaction effect.
#'
#' For more information, see the referenced article.
#'
#' @references
